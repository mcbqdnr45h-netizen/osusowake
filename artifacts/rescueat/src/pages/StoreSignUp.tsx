...
import { Eye, EyeOff, Mail, Lock, Store, CheckCircle2, ShieldCheck, User, Phone } from 'lucide-react';
...
export default function StoreSignUp() {
      ...
      <div className="min-h-dvh flex flex-col items-center justify-center bg-background px-6">
        ...
        >
          ...
          <h2 className="text-2xl font-black text-foreground mb-3">
            {needsConfirm ? '確認メールを送信しました' : 'アカウント登録完了！'}
          </h2>
          {needsConfirm ? (
            <>
  ...
  return (
    <AuthShell activeTab="store" mode="signup">
      ...
      >
        ...
        <h1 className="text-[28px] font-black text-foreground leading-tight tracking-tight">店舗オーナー登録</h1>
        <p className="text-muted-foreground text-sm mt-1.5 font-medium">
          お店の味を、もっと多くの人へおすそ分けしよう
        </p>
      </motion.div>
      ...
      <motion.div
        ...
        style={{ background: 'linear-gradient(135deg, rgba(255,140,0,0.06) 0%, rgba(255,180,0,0.04) 100%)' }}
      >
        <p className="text-xs font-black text-primary mb-2.5 tracking-wide uppercase">店舗オーナー特典</p>
      ...
      {/* ── フォーム ── */}
      <motion.form
        initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.38 }}
      ...
      >
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input type="email" value={email} onChange={e => { setEmail(e.target.value); setError(''); }}
              placeholder="store@example.com" className={inputBase} autoComplete="email" disabled={isLoading} required />
          </div>
        </div>
        ...
        <div>
          ...
          {!passwordsMatch && confirmPassword.length > 0 && <p className="text-destructive text-xs font-semibold mt-1.5">パスワードが一致しません</p>}
        ...
        <p className="text-center text-sm text-muted-foreground pb-2">
          すでにアカウントをお持ちですか？{' '}
          <Link href="/store/login" className="text-primary font-black underline underline-offset-2">ログイン</Link>
        </p>
      </motion.form>
    </AuthShell>
  );
