...
import {
  ...
  Minus, Plus, Power, BarChart3, Zap,
} from 'lucide-react';
...
export default function StoreOwnerDashboard() {
  ...
  const [updatingBag, setUpdatingBag] = useState<number | null>(null);
  ...
  const updateBag = async (bagId: number, patch: Partial<{ stockCount: number; isActive: boolean }>) => {
    setUpdatingBag(bagId);
    try {
  ...
  const handleStockChange = (bag: BagData, delta: number) => {
    ...
    setBags(prev => prev.map(b => b.id === bag.id ? { ...b, stockCount: newCount } : b));
    updateBag(bag.id, { stockCount: newCount });
  };
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 pb-24 space-y-4">
        ...
        >
          {[
            ...
            { label: '停止中', value: bags.filter(b => !b.isActive).length, icon: Power, color: 'text-muted-foreground bg-muted' },
            ...
            <div className="divide-y divide-border/60">
              {bags.map((bag) => (
                <motion.div
                  key={bag.id}
                  layout
                  className={`px-4 py-4 transition-colors ${!bag.isActive ? 'opacity-60 bg-muted/20' : ''}`}
                >
                  {/* 商品名 + 完売スイッチ */}
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        {bag.stockCount === 0 && (
                          <span className="shrink-0 text-[10px] font-black bg-orange-100 text-orange-600 px-1.5 py-0.5 rounded-md">
                            完売
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-0 bg-secondary rounded-2xl overflow-hidden border border-border">
                      <button
                        onClick={() => handleStockChange(bag, -1)}
                        disabled={bag.stockCount === 0 || updatingBag === bag.id}
                        className="w-12 h-12 flex items-center justify-center text-foreground hover:bg-muted active:bg-muted/70 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                      >
                      ...
                      <div className="w-14 text-center">
                        {updatingBag === bag.id ? (
                          ...
                          <span className={`text-2xl font-black ${
                            bag.stockCount === 0 ? 'text-orange-500' :
                    ...
                    <span className={`text-sm font-black ${
                      ...
                      {bag.stockCount === 0 ? '完売' : bag.stockCount <= 2 ? '残りわずか！' : '個'}
                    </span>
                  </div>
                </motion.div>
              ))}
