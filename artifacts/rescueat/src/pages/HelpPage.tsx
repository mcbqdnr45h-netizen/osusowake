...
const STORE_FAQ_SECTIONS: FaqSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        q: '「累計入金額」と「保留中の金額」が違うのはなぜですか？',
        a: '2つの金額は別の計算ロジックで表示されているため、必ずしも一致しません。\n\n【累計入金額（売上確認画面）】\nアプリが商品代金から理論計算した参考値です。「受取済み」の注文のみ集計しています。\n\n【保留中（出品管理 → 売上残高）】\n決済システム（Stripe）に実際に貯まっている残高です。受取前の取引も含まれ、これが実際の振込予定額になります。\n\n料金体系の改定をまたぐ過去取引が混在している場合、数円〜数十円のズレが生じることがあります。実際の振込額は必ず「保留中／振込可能」をご確認ください。',
      },
      {
        q: 'アカウントを削除したい場合は？',
