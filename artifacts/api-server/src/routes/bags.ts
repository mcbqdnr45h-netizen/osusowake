...
import { surpriseBagsTable, storesTable, insertBagSchema } from "@workspace/db/schema";
...
router.get("/bags", async (_req, res) => {
  try {
    const bags = await db
      ...
      .where(and(eq(surpriseBagsTable.isActive, true), eq(storesTable.isActive, true)));
